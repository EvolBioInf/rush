...
many programs available for doing this~\cite{pos02:eva,bru06:sim,mar10:rdp}. The
...
Next, explore average $Q$ and the rejection frequency ($\alpha=0.05$)
as a function of the rate of recombination in 100 pairs of 100\,kb
sequences.
#+end_export
#+begin_src sh <<cli>>=
...
#+end_src
#+begin_export latex
This can take up to a dozen seconds to generate the first result, so
be patient. Your results should eventually look similar to
\begin{verbatim}
#rho  <Q>        Rejection
0     -0.364891  0
1     0.111677   0.13
...
that there is no recombination signal left.
\begin{figure}
  \begin{center}
