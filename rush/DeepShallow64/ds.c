...
void ds_ssort(UChar *x, Int64 *p, Int64 n) { // 64-bit version
    ...
    for (j = 0; j < ALPHABET_SIZE; j++) {
      if (j != ss) {
		...
		if ( ! (ftab[sb] & SETMASK) ) {
			...
			if (hi > lo) {
				if (_ds_Verbose>2) {
					fprintf(stderr,"sorting [%d, %d], done %d " // llx and lld --> for long long variable j, numqsorted and
					"this %d\n", (int)ss, (int)j, (int)numQSorted, (int)(hi - lo + 1) ); // for variables hi and lo which are also of type long long
				} // end if
 ...
 if (_ds_Verbose) {
    fprintf(stderr, "\t %d pointers, %d sorted, %d scanned\n",
	    (int)Text_size, (int)numQSorted, (int)(Text_size - numQSorted)); // all variables are of type long long
    fprintf(stderr, "\t %d calls to helped_sort\n",(int)Calls_helped_sort);      
    fprintf(stderr, "\t %d calls to anchor_sort (forward)\n",
	    (int)Calls_anchor_sort_forw);      
    fprintf(stderr, "\t %d calls to anchor_sort (backward)\n",
	    (int)Calls_anchor_sort_backw);      
    fprintf(stderr, "\t %d calls to pseudo_anchor_sort (forward)\n",
    	    (int)Calls_pseudo_anchor_sort_forw);      
