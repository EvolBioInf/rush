...
void ds_ssorter(FILE *Outfile) {
  ...
  // control output
     ...
     for (pos = 1; pos <= n; pos ++ ) { // write first 50 char-s of suffix
       fprintf(Outfile, "%d %-50.50s\n", (int)sa[pos] + 1, text + sa[pos]); // suffixes are sorted from 1st position to n-th
       if (pos >= 10) {
         // before break, print the last suffix ; that is at most 10+1 suffixes 
         fprintf(Outfile, "\n\n%d %-50.50s\n", (int)sa[n] + 1, text + sa[n]); // suffixes are sorted from 1st position to n-th
  ...
  for(i=2;i<=n;i++)
    if(lcp1[i]!=lcp2[i]) { 
      ...
      if(Verbose>1)
        ...
        fprintf(stdout,"%s[%d]=%d  %s[%d]=%d\n", name1,(int)i,(int)lcp1[i],name2,(int)i,(int)lcp2[i]);
    }
  
  if(diff>0)
    //fprintf(stdout,"FATAL ERROR! %s/%s differences: %d\n",name1,name2,diff);
    fprintf(stdout,"FATAL ERROR! %s/%s differences: %d\n",name1,name2,(int)diff);
}
...
void printLCP(char *lcpType, FILE *OutfileLCP) {
  ...
  Int64 lcpLength; // the length of the lcp starting at the position pos  
  ...
  for (pos = 1; pos <= n; pos ++ ) { // write at most the first 50 char-s of the lcp; pos == 1 is undefined ??
    ...
    fprintf(OutfileLCP, "%12d %-50.50s %-50.50s\n", (int)lcpPrint[pos], text + sa[pos], lcpText); // lcp-s are sorted from 1st position to n-th
    ...
    if (pos >= 10) {
      ...
      fprintf(OutfileLCP, "\n%12d %-50.50s %-50.50s\n\n", (int)lcpPrint[pos], text + sa[n], lcpText); 
      printLine(OutfileLCP); // printh the separating line
      break; // this is just for testing; break after first 10+1 
     }
  }
